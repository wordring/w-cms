...
3. **添付（PDF・DXF）を出典として指せるか**——ユーザー（2026-08-31）:「PDFやDXFなどを
   出典としてリンクすることもあるので、これらもIDが必要かもしれません」。添付は
   ページのフォルダ内でファイル名が一意なので、素直な形は **`ページID-ファイル名`**
   （ブロックIDの代わりにファイル名）。ブロックIDと違い改名で切れるが、添付の
   改名UIは存在しないので実運用では安定している。設計時に確定すること。
