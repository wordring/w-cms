...
3. **添付（PDF・DXF）を出典として指せるか**——**未実装**。ユーザー（2026-08-31）:
   「PDFやDXFなどを出典としてリンクすることもあるので、これらもIDが必要かも
   しれません」。添付はページのフォルダ内でファイル名が一意なので、素直な形は
   **`ページID-ファイル名`**（ブロックIDの代わりにファイル名）。ブロックIDと違い
   改名で切れるが、添付の改名UIは存在しないので実運用では安定している。設計時に確定すること。
...
ワンノート実データの15種−.pdf＋.eml。**.json は書けない**）。最初の住人は
