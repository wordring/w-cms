...
| 判断 | 決定 | 理由 |
|---|---|---|
| **派生（DB）から正本（ファイル）へ書き戻さない**（**未実装**——現在の実装は書き戻す） | サイドカー `<id>.meta.json` が読めないとき、`page_perms`・`pages` に残っている値から**サイドカーを組み立て直して書き戻すことをやめる**。読めない事実をそのまま残し、**運用者が手作業で直す**。DBから正本への書き込みは、集計など**必要な場合に限る** | ユーザーの言葉:「**これは設計を変えます。サイドカーが読めない場合、運用者が手作業で修正します。自動で治すことはしません。データベースから正本への書き込みは集計など必要な場合に限られます。**」——§8.1 の不変条件（DBはいつでも捨てて作り直せる派生）の**裏返しの規律**にあたる。派生から正本を書くと、正本の欠損が派生の内容で上書きされ「**見た目は健全なのに中身が変わっている**」状態を作る。壊れたときに壊れたままにしておくほうが、気づける。**現在の実装はこの決定に反する**——`readMetaOrPerms` がサイドカーを読めないとき `GetPerms` ＋ `fillFromIndex` で `pages` から親・作成情報を拾い、続く `BumpUpdatedAt` / `SetSidecarParent` の `WriteSidecar` がそれを正本として書き出す（[page/perms.go](../internal/cms/page/perms.go)）。この経路は 2026-08-21 に「サイドカーが読めないと親ページIDを失う」バグを塞ぐために足したもので、**欠損の緩和としては正しく動くが、方針としては採らない**。直すときは `sidecar_loss_test.go` の期待も一緒に反転させること |
