...
>
> **予定（2026-08-16 決定・未実装）**: 語彙モデルの移行で、ここに**汎用索引テーブル**
> （全 `table[data-type]`/`dl[data-type]` の縦持ち同期先。完全正規化）がコアテーブルとして
> 加わる。**「コアはドメイン語彙を知らない」原則はそのまま**——コアにとって `data-type` の
> 値は不透明な文字列であり、`<m-*>` を知らないのと同じ構図。特定の型の意味を知るのは
> 従来どおり計算プラグインだけ。[【考察】語彙モデル.md](【考察】語彙モデル.md) §2 参照。
>
...
*   **レスポンス**: `{"success", "page_id", "block_id", "updated_at", "html"（サニタイズ後の当該ブロック）, "sanitized"}`。
    エコーバックもブロック単位になります。
...
*   **合成**: `RenderPageShell`（[shell.go](../internal/cms/shell.go)）が `assets/index.html` の
    ...
    テンプレートエンジンは使いません（巨大なインラインJSが contextual escaping で壊れるため）。
