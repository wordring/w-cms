...
## 7. 【アイデア・未確定】製品マスタページとバージョン管理

以下は2026-06-20に議論した設計アイデアであり、まだ採用は決定していない。実装に着手する際は本項目を正式な仕様として書き直し、確定後はこの「アイデア」見出しを外すこと。

### 背景・課題
従来のRDBMS的な固定スキーマではなく、本プロジェクトのコンセプト（[コンセプト.md](コンセプト.md) 3章）に沿って「製品マスタ（例: 鋼材の種類、発注時に必要な項目）」もページとして自由に記述・編集できるようにしたい。ただし、マスタページの内容を直接DBに同期する場合、マスタが書き換わると過去の発注済み明細（`client_order_items` 等）の参照内容まで事後的に変化してしまう問題がある。
...
2.  **マスタページの保存をバージョニングする**: マスタページが保存される際、既存の `SyncIndex` の同期対象に `materials` 的なマスタテーブルを追加し、UPDATEではなく新しい `version`（または世代番号）の行をINSERTする。
3.  **発注明細はバージョンをスナップショットする**: `<m-item>` を生成する時点で、参照したマスタの `material_id + version` を明細側に記録する。これにより、マスタが後から書き換わっても、発注済みの明細は発注当時の条件のまま固定され、表示や計算に影響しない。

### 未確定の論点
*   バージョン行をどのテーブルに持たせるか（既存の `part_materials` を拡張するか、新規テーブルを切るか）。
*   マスタページのどのタグ変更を「新バージョン」とみなすか（全属性の単純な差分比較か、特定フィールドのみ監視するか）。
*   旧バージョンの参照が必要な集計（例: 部材手配計算API）にどう影響するか。
