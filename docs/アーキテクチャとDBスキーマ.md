...
| 判断 | 決定 | 理由 |
|---|---|---|
| **硬いドメイン表8つを落とし、汎用索引へ一本化する**（D-1・**未実装**） | 受注・受注明細・発注・発注明細・販売見積・購入見積・部材・タグの8テーブルを廃し、`vocab_index` 1本へ寄せる（`page_tags` も中身が重複しているので吸収）。集計は索引から Go で束ねる | ユーザーの判断:「**する**」。週2枚の規模では縦持ちの弱点（性能）が発動しない。`単価（ロット20）甲田` のような**可変列名は横持ちでは受からない**、が実データの結論（[【考察】ワンノート移行.md](【考察】ワンノート移行.md) §3-5）。板金部の語彙がコアのスキーマから消え、「語彙とプラグインは運用者のもの」（[要件定義書.md](要件定義書.md) §1.1・§4.5）と一致する |
