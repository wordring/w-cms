...
書き込みは**一時ファイル＋rename**です。`WriteSidecar` が `os.WriteFile`（O_TRUNC）のままで
「書きかけの切り詰め」を作りうる（[作業引き継ぎ.md](作業引き継ぎ.md) の小粒な残り）のに対し、
設定は正本かつ再生成できないので、こちらは最初から原子的に書きます。
