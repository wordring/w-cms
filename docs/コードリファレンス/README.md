...
この版は D-1（2026-08-31・硬いドメイン表の全廃）以降の大改造——汎用索引 `vocab_index`
への一本化・機能見出し形（D-2）・参照タグの描画（D-4/D-10）・添付の `files/` 分離と
きれいなURL・汎用添付口・取り込み係（受信箱）・ボタン起動のPDF解析・重複検知——を
反映して**実際のコードから測り直した**ものです。
...
D-1 以降の大改造で再び古くなったため **2026-09-02 にもう一度作り直した**のがこの版です。
