...
### 4.1. バックアップ手順
*   **対象**: `data/master/` ディレクトリ全体（必要なら `data/` ごと）。
*   **対象外**: `data/cms.db`（バックアップしてもよいが必須ではない。復元時は再生成される）。
*   稼働中でも `data/master` 配下のファイルは追記・上書きのみで参照整合が壊れないため、ファイルコピーやrsyncでそのまま取得できます。

```bash
# 例: data/master をまるごとバックアップ
tar czf wcms-backup-$(date +%Y%m%d).tar.gz data/master
```
