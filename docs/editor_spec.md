...
### 7.3. 自動解析と明細追加
*   **推測APIのコール**: アップロード完了直後に、続けて `POST /api/parse-pdf` をバックグラウンドで呼び出します。
*   **DOMへの自動挿入**: バックエンドがPDF内から抽出したテキストをもとに明細データ（`items`）を返却すると、フロントエンドはそれをループ処理し、自動で複数の `<m-item>` タグを生成してDOMに挿入します。
*   **自動保存連携**: 明細が挿入された後、`window.updateHtmlPreview()` と `window.triggerAutoSave()` を呼び出すことで、自動的にユーザーの操作なしにデータが保存されるゼロクリックUXを実現しています。
