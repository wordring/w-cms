...
<body>
    ...
    <m-file src="attachments/po_shaft.pdf" name="正式発注書_南北.pdf" 
...
*   **SQLite FTS5 (trigram) による日本語全文検索機能の追加**
    *   データ量（ドキュメント数）が増加した段階で実装を検討します。
    *   HTMLファイルからタグを除外したプレーンテキストを抽出し、SQLiteのFTS5（全文検索エンジン）にインデックス同期することで、ミリ秒単位の日本語キーワード検索を実現します。
    *   導入時は、データベース定義を更新した後に「6. DB再構築（リビルド）機能」を実行するだけで、過去に作成したすべてのドキュメントに対しても一括で検索インデックスを生成可能です。

*   **Microsoft 365 (Microsoft Graph API) 連携によるメール自動取り込み機能の追加**
    *   受注窓口となる M365 メールアドレス宛てに届いたメールを、Goのバックエンドが Microsoft Graph API を用いて定期的に巡回監視（ポーリング）します。
    *   メールを受信すると、メール本文をテキストとして新規ページ (`[page_id].html`) に保存し、添付された発注書PDFを `attachments/` に自動配置します。
    *   その後、裏で Gemini API を呼び出し、メール内容およびPDFから自動的に発注書番号や顧客名、部品情報を読み取り、HTML上に `<m-file tag="顧客の発注書" order-no="...">` および `<m-item>` タグを自動生成してデータベースに同期します。
    *   これにより、顧客から発注書がメールで届くだけで、ユーザーが手動で登録することなく自動的に受注が記録され、進捗管理DBが更新される完全自動化システムを実現します。
