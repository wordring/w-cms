...
| POST | `/api/intake/handled` | 通信記録に**「対応：不要」のタグを付ける**（2026-09-05。`{page_ids:[…]}` でまとめて押せる）。未処理の一覧から1クリックで片付けるための口——付けるのは**コアの語彙だけ**で、「見積依頼にする」「受注にする」は業種の語彙なので拡張の仕事。**編集ロックは要らない**（人が編集中の本文を奪わず、タグを1つ足すだけ）。**1件の権限不足で全体を止めない**（応答は `{success, handled, failed}`）。監査記録は `intake.handled` |
