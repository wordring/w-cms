...
- **拡張の口** … 各プラグインの `Routes()`。`main.go` は `cms.PluginRoutes()` をループして
  ...
  | `ext/subcon`（下請け業務） | [subcon/materials.go](../ext/subcon/materials.go) | `/api/required-materials`・`/api/analyze-attachment`・**`/api/parse-pdf`**（2026-09-16 にコアから移設）・`/api/filing-proposal`・`/api/file-drawings`・`/api/analyzed`・`/api/filing-target`（09-20）・`/api/order-item`・`/api/order-pdf`・`/api/material-search`・`/api/our-order/new`（いずれも 09-21） |
...
| POST | `/api/analyze-attachment` | 要認証（対象ページの write） | **PDFの判定→受注ページ／加工製品ページ生成**（2026-09-01。下請け業務 [ext/subcon/analyze_pdf.go](../ext/subcon/analyze_pdf.go)——**`-tags minimal` で外れる**（2026-09-03。ルートもプラグインの `Routes()` 経由なので一緒に消える））。`{page_id, file}` で添付の `.pdf` を Gemini に（⚠ ZIP の中の PDF は 2026-09-17 から扱わない——取り込みが展開して中身を添付にするので、`entry` 引数は廃止） **3分類**させる（`doc_type` が `order` / `drawing` / `other`）。**枝は3つ**——①**発注書**（`is_client_order:true`）なら受注ページを対象ページの子として生成（`cms.CreateChildPage`。ヘッダ dl＋明細 table＋参照タグ `受信元: <ページID>-<添付ID>`）。応答 `{success, is_client_order:true, page_id, title}`・監査記録 `analyze-pdf` ②**図面**なら加工製品ページを生成（`<h2>図面</h2>` のヘッダ dl＋参照タグ＋**`<section data-type="file-view" data-ref="<ページID>-<添付ID>">`**＝図面をその場で開くマーカー（**配線は属性1つ**・2026-09-15。それまでは中に `受信元` のタグを書いていたが、同じ値がすぐ上の図面ブロックにも在り、2つの意味で並んでいた）。同じページのDXF添付と図面番号で突き合わせる `MatchDXFAttachments`）。応答 `{success, is_client_order:false, doc_type:"drawing", page_id, title, matched_dxf}`・監査記録 `analyze-drawing` ③**それ以外**は `{success:true, is_client_order:false}` で何も作らない（人が押した問いに「発注書ではない」と答えるのも正常な結果）。**起動は人の操作だけ**（📎の「🤖 解析」ボタン・人間ゲート型）。**ロックは要らない**（対象ページの本文は変えない）。**取り消しはページ削除**（ゴミ箱） |
| GET | `/api/filing-proposal` | 要認証（対象ページの read） | **整理の候補**（2026-09-03。下請け業務 [ext/subcon/filing.go](../ext/subcon/filing.go)——`-tags minimal` で外れる）。`?page_id=X` で通信記録ページ X の子のうち、**図面ブロックを持つもの**（加工製品ページ）と**発注書ブロックを持つもの**（受注ページ・2026-09-06）を集めて返す。応答は `{success, rows, orders, stages}`——`rows` は部品（`page_id, title, drawing_no, drawing_name, customer, machine_name, stage`。値は索引から読んだ**推奨値**で、人が直す）、`orders` は受注（`page_id, title, order_no, client_name, ordered_at, destination`。**直す欄は無い**——行き先は発注日で決まる）、`stages` は段の選択肢（設定 `extensions.subcon.machine_stages`）。読めないページは黙って落ちる（見せ分けC案） |
| POST | `/api/file-drawings` | 要認証（各ページの write） | **整理の実行**（2026-09-03。同上）。`{rows:[{page_id, customer, stage, machine_name, drawing_name, confirm_revision}], orders:[page_id,…]}`。**部品**は `取引先／社名／段／装置名称` の下へ移し、題を図面名称に揃える（`cms.SetPageParent`＋`SetPageH1`。途中のページは無ければ作る）。空欄の行と、段が `machine_stages` に無い行は動かさない。移した先に同名の加工製品ページが在れば**改定図面**として先頭へ合流（`confirm_revision` が要る場合は `needs_confirm` を返す）。**受注**は `受注／年／月` の下へ移す（年月は**発注日**・2026-09-06）——受注ページでないIDは動かさない。応答は `{success, results:[{page_id, outcome, message, target_id?}]}`（`outcome` は `moved` / `revision` / `skipped` / `needs_confirm`）。**権限も「編集中か」も行ごとに見ます**——駄目な行は `skipped` に載るだけで、残りは片付く（**複数のページへ書く**ので、関門はハンドラではなく行ごと・2026-09-14）。監査記録は `file-drawing.move`・`file-drawing.revision`・`file-order.move` |
| GET | `/api/filing-target` | 要認証 | **整理の行き先にページがあるかを聞く**（2026-09-20。`?customer=&stage=&machine=&name=`・[ext/subcon/filing.go](../ext/subcon/filing.go) の `FilingTargetAPIHandler`——`-tags minimal` で外れる）。打ち替えのたびに呼ばれ、`{success, exists, ...}` を返す。⚠ **調べるだけで1枚も作らない**——`ensureChildPage` は無ければ作るので使わず `findChildByTitle` だけで辿る（問い合わせのたびに空の顧客ページが増え、次の問い合わせで「在る」と答える最悪の副作用を避ける）。読めないページは「無い」と答える（見せ分けC案）。欄が埋まっていなければ `exists:false` |
| POST | `/api/order-item` | 要認証（対象ページの write・**編集ロックの関門**） | **受注明細のセル1つを書き換える**（2026-09-21。[ext/subcon/order_edit.go](../ext/subcon/order_edit.go)）。`{page_id, row, item_no, field, old, value}`——`row` はデータ行の番号（0始まり）、`item_no` はその行の `品番`（**行番号と2つで照合**）、`field` は列の見出しの表示文字、`old` はいまの値（**食い違えば断る**——誰かが先に直した）。受注ページの `弊社品番` を結ぶ画面の口 |
| POST | `/api/order-pdf` | 要認証（対象ページの write） | **発注書のPDFを作って、そのページの添付として残す**（2026-09-21。[ext/subcon/order_pdf.go](../ext/subcon/order_pdf.go)・`gopdf`・和文は TTF 埋め込み）。`{page_id}` → 開くための URL を返す。⚠ **7年保存がこれで済む**——ファイルがページと一緒に残り、版も監査も付く。差出人（担当者ページの署名）は未実装 |
| POST | `/api/material-search` | 要認証 | **材質・形状・寸法で材料を探す**（2026-09-21。[ext/subcon/dimension_search.go](../ext/subcon/dimension_search.go)・画面は `material-search` ビュー）。`{material, shape, thickness, diameter, values}`。⚠ **読めないページは混ぜない**——材料の表には誰でも行を書けるので、絞らないと読めない発注書の単価と仕入先が引けてしまう |
| POST | `/api/our-order/new` | 要認証（`発注` の置き場の write） | **未手配の一覧から発注書を1枚作る**（2026-09-21。[ext/subcon/our_order_new.go](../ext/subcon/our_order_new.go)）。`{supplier, order_at, due, note, lines:[…]}`。⚠ **仕入先が無ければ作らない**——1枚＝1社が発注書の単位（相手が決まっていないなら発注書ではなく手配メモ）。置き場は `発注／年／月`（発注は納期のグループから出るので受注ページには置けない・2026-09-21 決定） |
| GET | `/api/suggest` | 要認証 | **入力候補**（2026-09-21。`?source=…&q=…`・コア [internal/cms/suggest.go](../internal/cms/suggest.go)）。出どころは拡張が `cms.RegisterSuggestSource` で持ち込み、**コアは名前しか知らない**（`推奨業者` が最初の1つ・`ext/subcon/suggest_partner.go`）。⚠ **知らない出どころは 404 ではなく空で返す**——拡張を外したビルドではその出どころが居ないのが正常で、画面は候補が出ないだけであるべき（拡張の出し分けと同じ考え方） |
| GET | `/api/zip-list` | 任意認証（read） | **ZIP添付の目録**（2026-09-01。`?page_id=&file=`。`OptionalAuth` 配下）。標準の `archive/zip` でセントラルディレクトリだけ読み、**この口は展開しない**（ZIP爆弾対策。展開はメール取り込みの時点で `cms.ExpandZip` が上限つきで行う——2026-09-17）。UTF-8フラグの無いエントリ名は Shift_JIS として復号。最大500件（超過は `truncated:true`）。認可は添付配信と同じ閲覧側の関門（`RequirePageReadOrPublic`——読めるページの添付は目録も読める）。応答は `{success, entries:[{name,size}], total, truncated}` |
...
| POST | `/api/contacts/register` | 要認証（新規は `取引先` ページ——無ければトップ——の write／既存へ足すときはその相手ページの write） | **メールから拾った相手をページにする**（アドレス帳・2026-09-05。**2026-09-15 から拡張 [ext/comm/contacts](../ext/comm/contacts/) の `Routes()` 提供**——`-tags minimal` で外れる）。`{name?, page_id?, relation, person_name?, addresses:[…], domains?:[…]}`。**行き先の組織は3段で決まる**（2026-09-17）——① `page_id` で名指し ② `name` の**題の完全一致**で既にある組織（画面の組織コンボボックスに無い名前を打っても会社を2枚にしない・`PartnerByTitle`） ③ 無ければ `連絡帳` の下に新しく作る（題は `NormalizeNameForIngest` で畳む。本文は `取引：<relation>`＋`ドメイン`×n、担当者が無ければ `メールアドレス`×n）。`person_name` があれば**組織の直下の人のページ**へアドレスを入れる（無ければ作る）。`relation` は**任意**（2026-09-17。空なら `取引` のタグを書かない。表＝`顧客`／`仕入先`／`自社` に無い値は400）で、`domains` とともに**これを送る画面はもうありません**——どちらも組織の性質なので、必要になったとき組織のページでタグを1行書きます。`domains` は組織へ `ドメイン` タグを足す——⚠ **2026-09-17 からこれを送る画面はありません**（未登録の連絡先の行にあったチェックを外した。役に立つのは「新しい組織＋専用ドメイン」の1場面だけなのに、既定で入っていて共有ドメインを組織に結びやすかったため）。口は意図して残してある。`name` が `個人` なら**取引は人に付き、ドメインは付けない**——担当者が無ければ400。応答 `{success, page_id, title, added, domains_added, merged, created}`（`page_id` はアドレスが入ったページ＝人が居れば人）。**`editlock.RefuseWhileEditing` を通す**（本文を読んで・変えて・書くので、誰かが開いていれば409）。監査記録 `contact.register`／`contact.add-addresses` |
...
`/api/required-materials`・`/api/filing-target`・`/api/order-item`・`/api/order-pdf`・
`/api/material-search`・`/api/our-order/new` はすべてこの経路**で、`main.go` のルート表には
現れません（冒頭の内訳表）。`/api/suggest` だけはコアの口で、**候補の出どころ**を拡張が差し込みます。
