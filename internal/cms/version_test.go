...
// TestVersionsAreNeverDeleted は、⚠ **版を消す機構を持たない**ことを固定します。
//
// 2026-09-23 ユーザー決定:「**5年で消す動作をやめましょう**」
// 「**消さないの意味は、消す機構も持たないで良いです。もしも消す必要が出来たら、
// エージェントで消します**」。
//
// ⚠ **年限を1つに決められないことが分かったのが、この決定の中身です**——帳票の保持
// 義務は**個人5年・法人7年・過去に問題を起こしていた法人は最大10年**で、
// **設置した先がどれかは w-cms には分かりません**。⚠ **そして消えた版は戻りません。**
//
// ⚠ **「設定で0なら消さない」でもありません。** 機構そのものを持たないので、
// **設定の書き間違いでも消えません**——**消してよいと判断できる場所が
// コードの中に無い**、という形にしてあります。
func TestVersionsAreNeverDeleted(t *testing.T) {
	setupSaveTest(t)
	const id = "000040"
	...
	// ⚠ **別の編集者で保存します。** 同じ人が続けて保存すると
	//    **コアレッシングの枝へ入って早く戻り**、版を新しく書く道を通りません
	//    ——かつて掃除が付いていたのは**その道の最後**なので、同じ人で保存する
	//    番人は**掃除を戻しても落ちません**（2026-09-23 の変異試験で実際に空振り）。
	if err := RecordVersion(id, "べつのひと", "<h1>あと</h1>", false); err != nil {
		t.Fatalf("RecordVersionエラー: %v", err)
	}
	...
	if joined := strings.Join(got, "|"); !strings.Contains(joined, "20年前") {
		t.Errorf("⚠ 古い版が消えています（消す機構は持たないはずです）: %v", got)
	}
}

// writeFakeVersion は指定時刻の版を直接置きます（古い版が消えないことを見るため）。
