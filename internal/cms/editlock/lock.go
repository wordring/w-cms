// Package editlock はページ単位の**悲観ロック**（同時編集の直列化）を担います。
//
// 1ページ＝1書き手にすることで、本文の履歴が線形になり（マージ不要）、
// 保存の競合そのものが起きなくなります。設計は
// [docs/【考察】同時編集の競合対策.md] が正本で、要点は3つ:
//
//   - ロックは**揮発的**なランタイム状態（プロセス内の map）。単一プロセス前提で、
//     再起動で全ロックが消えるのは許容します。
//   - presence は **SSE 接続の有無**で判定します（ポーリングしない）。保持者の接続が
//     切れれば即座に明け渡せ、待機者が去れば猶予が取り消されます。
//   - 無競合なら無期限保持。**待機者が現れてから2分**の猶予で明け渡します
//     （競合トリガー方式——誰も待っていないのに時間で切らない）。
//
// ページの状態を変えるAPIは、この [RequireEditLock] か [Locks].Validate を通ります。
// 検証規約はどちらも同じで、**ロックが無ければ許可**（無競合）、保持者本人で
// トークンが一致すれば許可、それ以外は 409 です。
//
// [OnRelease] は保持者がロックを手放したときに呼ばれるフックで、cms パッケージが
// 版のチェックポイントを打つために使います（editlock は cms を import できないため、
// 関数変数で受けます）。
package editlock
