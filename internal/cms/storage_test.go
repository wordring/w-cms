...
func TestParseAndSyncNestedOrders(t *testing.T) {
	...
	<body>
		...
		<m-file src="attachments/po_test.pdf" tag="顧客の発注書" order-no="PO-T100" client-name="南北" ordered-at="2026-06-18">
	...
	if clientName != "南北" || pdfPath != "attachments/po_test.pdf" {
...
func TestRequiredMaterialsCalculation(t *testing.T) {
	...
	// 受注：SHAFT-01 を 10本
	_, err = db.Exec(`
		INSERT INTO client_orders (order_no, client_name, page_id) VALUES ('PO-A100', '南北', 2)
	`)
	if err != nil {
...
<m-file tag="顧客の発注書" order-no="PO-RB1" client-name="南北">
