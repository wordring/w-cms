...
//	③ 計算プラグイン … 索引を読んで集計する（ext/subcon/materials.go）。計算が要るときだけ書く
//
// # 本文を読むのは配送係だけ
//
// 本文の走査はコアの**配送係**（walk.go）が1回だけ行い、引き金（`data-type`）に当たった
// 要素を担当へ届けます。段は2つ——保存時の[Observer]（観察係）・表示時の[MirrorHandler]
// （鏡型）——で、それぞれ別のコンテキストを持ちます（新規作成時の種まきは 2026-09-25 に
// 撤去——テンプレートは純粋なコピーになった）。
