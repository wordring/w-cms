...
//
// **引き金の解決は1箇所（vocabTypeOf）に隔離**してあります。機能見出し
...
// VocabTypeOf は要素が名乗る形式名を返します（無ければ空）。
//
// **拡張が「配送係はこの表を自分で届けるか」を判断するための口**です（2026-09-21）。
// ⚠ **形式の解き方を2か所に持たないこと**——節の中の表を自分で拾う索引は、
// 「配送係が別に届ける表」を飛ばす必要があります。判定がずれると**同じ値が
// 二重に入り**、集計が黙って倍になります（同日、コア自身がそれを踏みました）。
func VocabTypeOf(el *html.Node) string { return vocabTypeOf(el) }
