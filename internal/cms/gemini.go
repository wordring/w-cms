...
// （docs/【考察】通信記録処理.md 2026-09-01 追記②）。
//
// 呼び出しの型（キーの取得・クライアント生成・応答テキストの合成・フェンス剥がし）を
// ここに1本化し、プロンプトは呼ぶ側（ParsePDFHandler・取り込み観察係）が持ちます。
//
...
// errNoGeminiKey は GEMINI_API_KEY 未設定の印です。呼ぶ側が「エラーとして知らせる」
// （手動のPDF解析）か「静かに諦める」（取り込みの自動判定）かを選べるよう区別します。
