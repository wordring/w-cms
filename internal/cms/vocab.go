...
type VocabDef struct {
	...
	// 鍵がある場合に宣言します。
	//
	// これが無かったころ、鍵は③計算の側（いまの ext/subcon/materials.go）に
...
//
// 山括弧が無ければ、全体をアドレスとみなします（`ozawa@example.jp` だけの書き方）。
...
//
// **助数詞を列挙しないのは、ここが number 型と宣言された列だからです。**
// 「数量」「単価」の升目に入っている以上、数のうしろに続く文字は単位か助数詞
// （個・本・枚・台・式・mm・rpm…）と読んでよく、知らない語のために取りこぼす
// ほうが損です。**text の升目には掛かりません**——品名の `170球` のような値が
// 数に化けることはありません。
