...
func TestEnsureContactPersonBuildsTree(t *testing.T) {
	...
	again, err := EnsureContactPerson(user, companyID, "山田 太郎")
	if err != nil {
		t.Fatal(err)
	}
	if again != personID {
...
func TestPartnerTitleForAddressFindsPersonPage(t *testing.T) {
	...
	if _, err := AddContactAddresses(personID, user.Username,
		[]string{"yamada@example-sports.co.jp"}); err != nil {
		t.Fatal(err)
	}

	// ① 完全一致——担当者ページに載っていても、答えるのは会社の名前。
	if title, ok := PartnerTitleForAddress(user, "yamada@example-sports.co.jp"); !ok ||
		title != "南北スポーツ機械" {
