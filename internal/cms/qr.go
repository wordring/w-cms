...
// `qr_test.go` が**独立実装（Python の segno）で作った行列と1ビットずつ突き合わせます**
// （`testdata/qr/*.txt`）。マスク選択・誤り訂正・型番選びまで全部含めて一致するので、
// 「なんとなく動いている」ではなく**同じ絵が出る**ことが固定されています。
//
// 参照実装は開発機の照合用で、**製品は何にも依存しません**。
// ─────────────────────────────────────────────────────────────────────────
