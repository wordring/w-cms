...
func TestIntakeFallbackTakesAnyFile(t *testing.T) {
	...
	pageID, title, err := h.OnFile(ctx, "A100-B01-0007_組立ベース台座.dxf", []byte("0\nSECTION\n"))
	if err != nil {
		t.Fatalf("取り込みエラー: %v", err)
	}
	if title != "A100-B01-0007_組立ベース台座" {
		t.Errorf("題がファイル名から採られていません: %q", title)
	}
	body, err := os.ReadFile(filepath.Join(page.GetPageDir(pageID), pageID+".html"))
	if err != nil {
	...
	for _, want := range []string{"<dt>取り込み日時</dt>", "<dt>内容ハッシュ</dt>", `download="A100-B01-0007_組立ベース台座.dxf"`} {
		if !strings.Contains(html, want) {
			t.Errorf("ページに %q がありません:\n%s", want, html)
		}
	}
...
func TestIntakeKeepsExtension(t *testing.T) {
	...
	pageID, _, err := intakeHandlerFor(".dxf").OnFile(ctx, "P103-227-6_台座Assy.DXF", []byte("0\nSECTION\n"))
	if err != nil {
		t.Fatalf("取り込みエラー: %v", err)
	}
	entries, err := os.ReadDir(page.AttachmentDir(pageID))
