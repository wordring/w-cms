...
//
//  1. `ext/mail` からアドレス帳への参照は**ゼロ**でした。使っているのは
//     `ext/sheetmetal` の整理と、API のルート登録だけ。
//  2. 拡張どうしは import しない決まりだったので、`ext/mail` に置くと
//     **業務側から呼べません**。
//  3. `-tags nomail` で整理が壊れます——`ext/sheetmetal` は `!minimal` なので
...
// ── ビルドタグは付けません ─────────────────────────────────────────
//
// **使う側が連れてきます**（`ext/sheetmetal` が import する）。タグを増やすと
