...
var businessVocab = []cms.VocabDef{
	...
	},
	{
		...
		Columns: []cms.VocabColumn{
			...
			// 「単位はみなと商店くらいしか使っていないのですが、**残念ながら弊社の表に
	...
	},
	{
		...
		//	塗装・鍍金  … 商品コード｜商品名｜**色**（→ `表面`）｜単位｜数量｜単価｜金額（税抜）
		//
		// ⚠ **塗装と鍍金は完全に同じ形**で、**加工はそこから `表面`（旧 `色`）が抜けただけ**でした。
		...
		// 材料の発注書に `表面` は出ません（受注残表の印刷で作った仕掛けと同じ考え）。
		//
	...
	},
	{
		...
		// 人が書く表。発注フォルダの必要部材表の上に置き、行は必要部材表に並ぶ
		// （temp_parts.go）。⚠ **行が在る＝まだ発注していない**——発注部材表へ入れたら
		// 行は消え、弊社品番の無い行を戻すとここへ戻る（**移す・引き算しない**）。
		Type:        TempPartsType,
		DisplayName: "臨時部材表",
		Category:    "業務",
		Icon:        "🧺",
	...
	{
		// 必要部材表（2026-09-21 に「未手配の一覧」として作り、09-24 に改名——ユーザーの
		// 文書と「必要部材表へ戻す」ボタンに揃えた）。⚠ **受注を横断します**——発注は納期のグループ
...
func orderItemColumns() []cms.VocabColumn {
	return []cms.VocabColumn{
		...
		// ⚠ **`色` から `表面` に改めました**（2026-09-23 ユーザー:「**発注における色は
		//    表面に変えて、メッキや生地などもここに入れては**どうでしょう」）。
		//    塗装は `緑`、鍍金は `三価ユニクロ`、何もしないなら `生地`——**どれも
		//    「表面をどうするか」の答え**で、`色` では鍍金と生地が入りません。
		//    ⚠ **`Field` は `color` のままです**——機械キーを変えると、**既にある
