...
// PurchaseOrderBoxTitle は**弊社の発注書**の置き場（トップ直下）の名前です。
//
// ⚠ **2026-09-21 に決定を覆しました。** それまでは「**材料発注の控えは受注ページ**」
// （同日の決定・見るのは事務が受注ごとだから）でしたが、ユーザーの訂正で崩れました:
//
//	「弊社の発注書は、**受注明細の単位とは無関係に、納期のグループなどから発行**
//	 されます。したがって、**受注ページに置くことは出来ず**、しかし、受注明細の
//	 各行に**どの発注書で発注したかを記録しリンク**する必要があります」
//
// ⚠ **発注の単位が受注をまたぐ**なら、受注の下には物理的に置けません
// （1枚の発注書が2つの受注ページの子にはなれない）。形は受注と同じ
// **`発注／年／月`**（年月は発注日）で、⚠ **結びは参照**で作ります。
const PurchaseOrderBoxTitle = "発注"
