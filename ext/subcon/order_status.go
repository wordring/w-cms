...
//	発注済 … 出した（メール・FAX・手渡し）
//	納品済 … 届いた
//	取消   … **取り消した**。**その部材はもう発注しない**（2026-09-23 に意味が変わった・下）
//
...
// ── 読み手が守ること ──────────────────────────────────────────────────
//
// ⚠ **`取消` の行も「手配した」に数えます**（2026-09-23 ユーザー:「状態を取り消しに
// することの意味が、**その部材はもう発注しない**ということになりました。**発注書
// ページで消費して発注しなくなります**」）。⚠ **09-22 は逆でした**（数えない＝取り消すと
// 必要部材表へ自動で戻る）。**戻したいときは行末の「必要部材表へ戻す」**で、行ごと
// 発注書から外します（外れた行はどこにも数えられないので、必要部材表に戻ります）。
// ⚠ **`取消` の行は紙には刷りません**（合計にも入れない・`readOrderDoc`）——
// 「数える」のは手配の引き算だけです。
// ⚠ **`未発注` も数えます**——紙はできているので、**二重に発注しないため**です。
