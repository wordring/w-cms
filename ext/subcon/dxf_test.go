...
// DXF の表題欄読み取りのテスト。
//
// 実物2枚（`P103-227-6 台座Assy`・構想図）で確かめた振る舞いを、合成した最小の
...
func TestParseDXFTextsDecodesShiftJIS(t *testing.T) {
	body := dxfEntity("TEXT", sjisBytes(t, "図面番号"), 100, 50, 2.5) +
		...
		dxfEntity("MTEXT", "{\\fMS ゴシック|b0|i0;"+sjisBytes(t, "台座Assy(溶接図）")+"}", 100, 40, 2.5)
...
func TestDXFTitleBlockPairsLabelAndValue(t *testing.T) {
	...
	body := dxfEntity("TEXT", sjisBytes(t, "図面番号"), 309.3, 37.3, 2.06) +
		dxfEntity("TEXT", "P103-227-6", 311.1, 32.7, 2.06) +
...
// **丸ごと空振り**していました（PFC2・M305 の2様式で10項目とも0件）。
