...
func TestSentRecordUsesIntakeTagNames(t *testing.T) {
	body := sentRecordBody("minami@example.jp", "010272", "<parent@example.jp>", "<me@example.jp>",
		time.Date(2026, 9, 15, 10, 0, 0, 0, time.Local),
		ReplyRequest{
			Subject: "図面の件",
			To:      []string{"ozawa@example.jp, 田中 <tanaka@example.jp>"},
			Cc:      []string{"cc@example.jp"},
			Body:    "よろしくお願いします。",
		})

	for _, want := range []string{
		"<dt>" + comm.FromTag + "</dt><dd>minami@example.jp</dd>",
		"<dt>" + comm.ToTag + "</dt><dd>ozawa@example.jp</dd>",
		"<dt>" + comm.ToTag + "</dt>",
