...
//
// ⚠ **以下の段落は 2026-09-03 の判断で、上のとおり撤回済みです**（記録として残します。
// 2026-09-16 に「現行と読めてしまう」ことに気づいて印を付けました——上で「子ページの
// 有無でも決めない」と書いた直後に、その逆が地の文で続いていました）:
//
// > **判定に新しい入力を求めません。** ユーザー:「メールを読んだ時点で受注ページや
// > 加工製品ページが作られるので問題ない」——つまり**子ページが在ること自体が「手を付けた」の印**
// > です。人に状態を打たせると、打ち忘れたものが未処理として残り続け、一覧が信用されなく
// > なります。年フォルダ・月フォルダが並ばないのも同じ理由で済みます（子を持つため）。
//
// **いまの実装は `対応` タグだけを見ます。** 年・月フォルダが並ばないのは子の有無では
// なく**題で見分けている**からです（`IsDateFolderTitle`）——撤回と一緒に根拠も
// 入れ替わっているので、上の段落の最後の1文だけを信じないこと。
//
	...
	const q = `
		SELECT p.id,
		       ...
		       COALESCE((SELECT c.value FROM page_tags c
		                  WHERE c.page_id = p.id AND c.name = ? LIMIT 1), ''),
...
// チャネルを選択してはどうでしょうか」）。**ファイルはできたページへ落とします**
// ——PDF/DXF の自動取り込みは 2026-09-05 に取り止めました（機械が経路を決めつけていた）。
//
