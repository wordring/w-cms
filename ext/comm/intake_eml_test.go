...
func TestEmlIntakeCreatesRecordPage(t *testing.T) {
	...
	eml := "From: " + "=?ISO-2022-JP?B?" +
		base64.StdEncoding.EncodeToString([]byte(iso2022jp(t, "南北スポーツ"))) + "?= <toa@example.jp>\r\n" +
...
func TestEmlIntakeSplitsAddresses(t *testing.T) {
	...
	eml := "From: " + "=?ISO-2022-JP?B?" +
		base64.StdEncoding.EncodeToString([]byte(iso2022jp(t, "山田 太郎"))) +
		"?= <yamada@example.co.jp>\r\n" +
		// 宛先は2人（多値は対の繰り返し）。1人目は表示名つき、2人目は素のアドレス。
		"To: \"南 様\" <admin@example-works.co.jp>, order@example.co.jp\r\n" +
		"Cc: cc@example.co.jp\r\n" +
