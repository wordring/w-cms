...
// ─────────────────────────────────────────────────────────────────────────
// メール送信の口——通信拡張は「口」だけを持つ（2026-09-03）
//
// ⚠ **2026-09-15 にこのファイルはコアから `ext/comm` へ出ました。** 下の
// 「コアが口を宣言」は**当時の構図**です。いまは「**通信拡張**が口を宣言／
// `ext/comm/mail` が中身」で、口ごと `-tags minimal` で消えます。
// 形（口と中身を分け、使う側は口に尋ねる）は変わっていません。
//
