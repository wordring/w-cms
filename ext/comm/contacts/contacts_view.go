...
// ②登録済みアドレスのドメインから推測した組織 ③**表示名と法人格違いで一致する組織**
// （2026-09-20）④「個人」（無ければ新規として）。
//
...
func orgCandidates(user *auth.User, c UnknownContact) []orgCandidate {
	...
	// **法人格違いの同じ組織が連絡帳に居ないか**（2026-09-20 ユーザー:「株式会社や
	// 有限会社、（株）などを無くした社名が連絡帳にあれば、それを提案するような形で、
	// 編集者の承認を得てはどうでしょう？」）。
	//
	// ⚠ **ここが表記ゆれの発生源でした。** 同じドメインの組織が1つも無いとき、欄の初期値は
	// メールの表示名そのまま（`株式会社南北スポーツ機械`）になります。連絡帳に
	// `南北スポーツ機械` が居ても口は**完全一致**でしか寄せないので（`PartnerByTitle`）、
	// 押した瞬間に**会社ページが2枚**になります。しかも整理が2つの木を結ぶのも完全一致なので
	// （`linkPartner`）、**黙って結ばれないまま**になります。
	//
	// 候補に足すだけで初期値も直ります——`contactRowInitials` は「『個人』以外が
	// ちょうど1つならそれ」を初期値にするからです。⚠ **足すのは候補で、決めるのは人**:
	// 欄は編集できるコンボボックスのままで、打ち替えればそのとおりに登録されます。
	// 候補が2つ以上に割れるときは `SuggestOrg` が黙ります（`株式会社あさひ` と
	// `有限会社あさひ` は**実在しうる別会社**）。
	//
	// 同じ組織がドメインからも見つかっていれば、`add` が ID で畳むので増えません。
	if looksLikeCompany(c.SuggestName) {
		if s, ok := SuggestOrg(user, c.SuggestName); ok {
			add(s.PageID, s.Title)
		}
	}
