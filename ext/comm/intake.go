...
//
// 既存の段（観察係＝保存時・鏡型＝表示時。⚠ 種まき＝新規作成時は 2026-09-25 に撤去）に、
...
// 1つの実装を設定違いで複数の拡張子へ登録する場合のための口です。
//
// ⚠ **いまこの引数を使う呼び手はいません**（2026-09-16 に確認）。使っていたのは
// `fileIntake`（落ちてきたPDF・DXFを `チャネル：FAX` として取り込む係）ですが、
// **2026-09-05 に取り止めました**——「通信箱のPDF、DXF取り込みはやめましょう。
// メモに添付するようにしましょう」（機械が経路を決めつけていた）。口を残すのは
// FAXサーバーを繋ぐときの機械専用の受け口で要りうるためで、**死にコードとして
// 消さないこと**。同じ理由を書いた `intake_file.go` への参照が残っていましたが、
// そのファイルは実装ごと消えています。
