...
func TestMatchDXFInsideZip(t *testing.T) {
	...
	putAttachment(t, id, "zip001.zip", zipWith(t, map[string]string{
		"図面一式/P103-227-6.dxf": dxfWithTitle(t, "P103-227-6", "台座Assy"),
		"図面一式/R310-002.dxf":   dxfWithTitle(t, "R310-002", "本体"),
		"図面一式/読んでね.txt":       "DXFではないので無視される",
	}))

	got := MatchDXFAttachments(id, "P103-227-6")
	if len(got) != 1 {
	...
	if got[0].Entry != "図面一式/P103-227-6.dxf" {
		t.Errorf("ZIP内のパスが返っていません: %q", got[0].Entry)
	}
	if got[0].Name != "台座Assy" {
...
func TestMatchDXFZipAndLooseTogether(t *testing.T) {
	const id = "000012"
	setupExtTest(t, id, page.PageMeta{Owner: "alice", Mode: "330"})
	putAttachment(t, id, "dxf001.dxf", []byte(dxfWithTitle(t, "P103-227-6", "台座Assy")))
	putAttachment(t, id, "zip001.zip", zipWith(t, map[string]string{
		"P103-227-6.dxf": dxfWithTitle(t, "P103-227-6", "台座Assy"),
	}))
